...
**Tests live next to what they test** — `src/db/settings.ts` → `src/db/settings.test.ts`. Not a
mirrored tree, not a top-level `test/`. A test beside its subject gets read when the subject is
edited; one in a parallel tree does not, and drifts.

`src/__tests__/` is for shared harness code and the decision checklist only — never for tests of a
module that has a home of its own.
...
**Every file opens with a doc block, and every exported symbol carries one.** Not a restatement of
the signature — what it is for, and what a reader would otherwise get wrong.

- **Files** — a `/** ... */` at the top saying what this module is responsible for, and what it
  deliberately is not.
- **Exports** — functions, types, constants. Say the purpose, the non-obvious constraints, and any
  ordering or timing the caller has to respect. `@param`/`@returns` only where the name does not
  already carry it; `@example` where the usage is not guessable.
- **In `core/`, name the decision.** A rule implementing a plan decision cites it — `Implements D26:
  a wake-alarm exit-stop is provisional...`. The plan's decision table is the checklist for tests
  (D37); doc blocks are what make it the checklist for *code*. When behaviour and plan disagree
  later, that reference is the only thing that says which one moved.

Skip the block only where it would purely restate the name, and never in `core/`.

**This is written down because it cannot be picked up by imitation.** Mature repos get consistent
doc blocks for free — agents match the surrounding code. A repo starting at zero has nothing to
match, so the first files set the convention permanently. Write them from the first file.
...
**`docs/development/expo-guidelines.md`** holds what has already been retrieved — project layout,
dev builds, prebuild/CNG, config plugins — each pinned to the date it was checked. Read it first, but
treat it as a cache, not an authority: if a claim there looks version-shaped and matters to what you
are about to do, re-retrieve and update the file. Anything native that is added by hand in Xcode is
erased by the next `expo prebuild --clean`, so entitlements, usage strings and App Groups belong in
the app config or a config plugin.
