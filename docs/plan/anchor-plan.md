...
- **Setup honesty is the user's job.** Putting the dock on the far side of the room and the morning
  ...
  make the honest setup work well.
...
own enable switch. Neither depends on the other having run.
...
that clears it must live outside the bedroom.
...
location alongside it would drain the battery flat — on the one morning nobody is there to notice.
Cap the high-accuracy window at a few minutes from first ring, then fall back to coarse monitoring.
A departure that has not happened in the first few minutes is not the case this rule exists for.
...
say so. That it also happens to close a loophole is a side effect, not the reason.
...
violate them**. Build steps 1–2. No behaviour, no AlarmKit, no NFC, no Apple account.

**It launches.** Phase 0 ends with an app on the phone opening to a status screen that shows the
migrated schema version, a working database round trip, and the build's SDK versions — enough to
prove the stack end to end. That screen is `src/app/index.tsx`, and Phase 1 grows it into the home
screen rather than replacing it, so D36 holds from the first phase and not merely the second.

Four things must exist before the first line of `core/`, because each is trivial now and
retroactive surgery later:
...
- **`docs/development/expo-guidelines.md`** — Expo's own conventions, retrieved and pinned to a date.
  It belongs in Phase 0 because this is when the scaffold decisions get made, and an agent with
  nothing to read will scaffold from model memory instead.
...
and the configurable radius (D14). Needs Phase 1's alarm lifecycle to attach to.
...
~490 over its last seven — but that measures velocity on a **familiar codebase in a familiar
stack**, which is the opposite of Phase 1 here. Treat it as evidence about the specified-logic
parts, not about the total.

**Where that speed actually lands:** `core/` and its tests, the schema and migrations, golden
traces, UI screens — anything already specified in this document, which is most of Phase 1 and 3.
**Where it does not:** debugging a community native bridge that misbehaves, tuning beacon TX power
by trial, and anything requiring a device in a place. Phase 4 barely moves.
...
allowed to ring anything. Geofence behaviour can only be checked by actually going out and coming
back. Those are calendar time, not effort, and they run in parallel with other phases rather than
blocking them.
...
it*, and those are different things: the first is a build property, the second is the only way to
learn whether walking to a tag actually gets you up. That answer shapes how much the later phases
need to do, and it is unavailable from the code.
...
**During the session**, a BLE beacon at the dock is monitored. If the phone leaves the beacon's
range and stays gone past a debounce window, the dock alarm resumes. This is what makes early
docking safe: without it, docking at 20:00 and retrieving the phone at 20:05 defeats the whole
feature.
...
reference Feature B's alarm time: the features are independent (D10), Feature B may be disabled
entirely, and a dock session must work with no morning alarm configured at all.
...
is: on the break, post the notification and immediately *schedule* an AlarmKit alarm for `now +
grace`, then cancel it if the phone docks in time. The waiting is the OS's job. A `setTimeout` here
would simply never fire, and the failure would be silent.

This exists so that coming home at 01:00 does not blast a house full of sleeping people for
something only the phone's owner cares about. The scheduled bedtime alarm has no grace; it fires at
a time the household already expects.
...
in history — never by refusing. Friction is a deterrent; a locked door is a defect.
...
currently ringing" — the rule in §11 is written per-role for exactly this reason. Overlapping
schedules should also be flagged at configuration time, since it is almost always a mistake.
...
hour at the boundary. Crossing a timezone re-bases the schedule to local time; places do not move,
so away-from-home behaviour (D9) applies as usual.
...
| Alarm (iOS) | `expo-alarm-kit` or `react-native-nitro-ios-alarm-kit` | Community packages — pin exact versions. **iOS deployment target 26.1** — the bridge's podspec declares `:ios => '26.1'`, and at 26.0 autolinking skips the pod *silently*: the build succeeds with zero errors and the module is simply absent, surfacing only at runtime as `Cannot find native module 'ExpoAlarmKit'`. Observed, not theorised. `expo-alarm-kit` exposes `configure(appGroupIdentifier)`, `scheduleAlarm` (one-shot), `scheduleRepeatingAlarm` (weekly, weekdays **1–7**), `cancelAlarm`, `getAllAlarms(): string[]` and `getLaunchPayload()`. The Nitro module needs a physical device and no-ops entirely on Android |
...
imports, no I/O. That is what makes a golden trace reproducible.
...
**`recordOccurrence` is how `fired_at` gets written.** D25 calls it the single most important thing
to record and D29's miss detection is built on it, so it cannot be an incidental side effect of some
adapter. It is an effect like any other: it appears in golden traces, `core/` owns *when* it
happens, and something outside performs the write. Ordering follows §12 — the row is written before
the consequence is acted on, so a crash in between leaves a recoverable intent rather than a lost
one.
...
saying only "wrong tag", is the worst feedback the app can give — it happens at 07:00, to someone
holding the right object, with no way to work out what is wanted. `notEnoughSteps` should say how
many more.
...
a test and replayed in the night simulator.
...
the user already knows the override exists, and that is precisely the night they are least equipped
to go looking for it. Later phases extend onboarding with place setup and beacon pairing.
...
title and one button that opens the app, and the app presents both. Pressing Stop re-arms (D2).
...
**Time is stored two different ways on purpose.** Schedules are `hour`/`minute` integers —
wall-clock local, so 07:00 stays 07:00 across a DST boundary (D23). Instants (`fired_at`,
`started_at`) are unix epoch milliseconds in UTC. Mixing the two is precisely how alarm apps drift
...
- **Editing an alarm invalidates its unresolved future occurrences**, exactly as a timezone change
  ...
  instants scheduled and the new time purely cosmetic.
...
backups by default**. Tag registrations, places and history survive a restore onto a new phone — the
tags themselves are physical objects and their UIDs do not change. Do not set the
exclude-from-backup flag to save space.
...
**This depends on the bridge exposing a list-scheduled call, which is not guaranteed.**
`AlarmManager` has `alarms` natively (§13 step 5 reads the SDK directly), but the community wrapper
may only surface schedule and cancel. **Verify it in the step 5 spike** — reconciliation as designed
...
and tested before any account exists. NFC and BLE cannot: the simulator has neither radio.
...
**Apply for the AlarmKit entitlement the day the account exists**, then carry on with steps 1–3
while it queues. Serialising behind it wastes the wait; ignoring it risks building `core/` for a
capability that never arrives.

1. Repo conventions + agent tooling (done).
2. **Phase 0 — the template.** Scaffold, configure, and make the rules enforceable.

   **Start from official scaffolding, not a hand-rolled tree** — but note the repo already exists and
   is not empty, and `create-expo-app` refuses a non-empty target. Scaffold aside, then merge in:

   ```
   npx create-expo-app@latest /tmp/anchor-scaffold --template default   # expo-router + TypeScript
   # copy in: src/, app.json, package.json, tsconfig.json, *.config.js, assets/
   # keep ours: CLAUDE.md, AGENTS.md, README.md, .gitignore, docs/, .claude/
   ...
   **`expo-location` is not installed here.** It has no Phase 0 job — `expo-sqlite` already exercises
   prebuild's native path — and an unused native dependency is one more variable in the build whose
   whole purpose is to prove the baseline. It arrives in Phase 2 with the code that uses it.

   **Set `ios.bundleIdentifier` before the first prebuild.** Prebuild needs it, and it is the identity
   the App Group and the AlarmKit entitlement hang off later, so changing it afterwards means
   re-provisioning. Pick the real one now.

   **Take the template's layout rather than imposing one.** On SDK 55+ the default template already
   ships a top-level `src/` holding `app`, `components`, `constants` and `hooks`, so routes live in
   `src/app` under the same root as `core/`, `alarm/` and `db/` — which is what §10's tree wants
   anyway. `src/ui/` is shared components, not screens. Config files stay at the root, and never
   keep both a root `app/` and a `src/app`: the latter silently wins.
   ...
   **Verified at build time, having been asserted from recall.** `better-sqlite3` has `db.serialize()`
   but **no** `Database.deserialize()` — restoring a snapshot is `new Database(buffer)`, and §15 said
   otherwise until this step corrected it. `useMigrations` does exist in
   `drizzle-orm/expo-sqlite/migrator`. Re-check both if the packages move; they carry the test harness
   and the status screen's migration row respectively.

   **NativeWind and Drizzle are the two whose setup must be retrieved, not recalled.** Neither is in
   the block above for that reason — both change their install steps between minor versions, and a
   stale `babel.config.js` or `metro.config.js` fails as an unrelated-looking error. Use the Expo MCP
   server and `llms.txt` (§9), and record what you land on in
   `docs/development/expo-guidelines.md`.

   **The thirteen files Phase 0 writes by hand.** Everything else comes from the template:
   ...
   | `drizzle.config.ts` | `driver: 'expo'`, schema and output paths. `db:generate` does nothing without it |
   | `src/db/schema.ts` | **`app_settings` only** — the §12 singleton. Every later phase needs it, it is not a throwaway to migrate away from, and its `CHECK (id = 1)` exercises a real constraint. The rest of §12 arrives with the phases that use it |
   | `src/core/types.ts` | The §10 vocabulary — `State`, `Event`, `Effect`, `Context`, `reduce`'s signature. Types only, no behaviour. Without it the test stubs cannot be written in the shape they will be filled in, and the purity rule guards an empty folder |
   ...
   | `tsconfig.check.json` | Extends the template's `tsconfig.json` and widens `include` to everything the gate must type-check — `scripts/`, `test/`, config files. The template's own config targets the app, so `type-check` would otherwise pass while the harness is broken |
   | `vitest.config.mts` | Node environment, no jsdom. `core/` is pure and the DB tests use `better-sqlite3`, so nothing here needs a browser-shaped runtime. The `.mts` extension is not cosmetic — as `.ts`, Vite loads it as CommonJS and warns on the ESM syntax |
   | `postcss.config.mjs` | NativeWind v5 runs Tailwind through PostCSS. v4's `withNativeWind({ input })` no longer applies — retrieved, not recalled |
   | `nativewind-env.d.ts` | Declares `*.css` and `*.sql` modules. Without it the side-effect stylesheet import and the generated migration bundle are both type errors |
   | `src/db/settings.ts` | Accessors for the singleton, **taking a db handle** rather than importing the device one — that is what lets the same code run under `better-sqlite3` in tests |
   | `src/db/client.ts` | `PRAGMA foreign_keys = ON` on every connection. Drizzle will not do it and SQLite defaults to off |
   | `src/__tests__/db.ts` | The same pragma, plus the migrate-once / serialize / restore template (§15). The pragma is per-connection and is **not** carried in the snapshot, so setting it only on the template leaves every test running with foreign keys silently off |

   **Plus the two scripts the gate is made of**, since `npm run check:code` cannot pass until they
   exist:

   | Script | What and why |
   | --- | --- |
   | `scripts/check-code.mjs` | The gate — lint, types, test, doctor, run concurrently with grouped output and a timing summary, per §15. `expo-doctor` degrades to a warning offline |
   | `scripts/check-house-rules.mjs` | The advisory scan (§15). Always exits 0, never gates |
   ...
   **What it launches into — the status screen.** Phase 0 ends with a real app on the phone, opening
   to one screen that proves the whole stack end to end rather than saying hello:

   - **Migration state** — the schema version the device actually migrated to. Proves the bundled-
     migration chain, the Metro and Babel config, and `PRAGMA foreign_keys` in one line.
   - **A round trip** — a control that writes a row through Drizzle and re-reads it. Opening the
     database proves less than it looks; writing and reading back proves the driver works on device.
   - **Build info** — Expo SDK, React Native and iOS versions, read at runtime. This is what you will
     want the first time a native module misbehaves.
   - Styled with NativeWind on the 4-point scale, so the styling pipeline is visibly working and not
     merely installed.

   **It is `src/app/index.tsx`, and it is not a throwaway.** Phase 1 grows this same route into the
   home screen of §10's surface table — the one showing both alarms and whether the last one actually
   fired (D25/D29). Phase 0 builds the frame that later phases fill, which is what makes D36's "every
   phase ends usable" true from the first phase rather than the second.

   **No paid account needed.** It runs in the simulator with no account at all, and on your own phone
   under a free Personal Team — Phase 0 touches none of the capabilities that are gated (Core NFC,
   App Groups, AlarmKit on device). The 7-day provisioning expiry means re-signing weekly, which is
   fine for a scaffold and is exactly why the paid account is needed before real use.

   **Phase 0 ships one real test, because an empty suite is not a passing one.** `vitest --run` exits
   non-zero when it finds no test files, so `npm test` cannot go green on an empty `core/`. Rather
   than paper over that with `--passWithNoTests`, Phase 0 writes the test that proves the harness
   itself: migrate the template database, assert `foreign_keys` is on, insert and read back through
   Drizzle, and assert the `id = 1` CHECK rejects a second row. That is the harness every later phase
   depends on, verified rather than assumed.

   **It also stubs the rest.** D37 makes §5's decision table the test checklist, so Phase 0 writes
   that checklist out as `it.todo(...)` stubs — one per decision describing runtime behaviour,
   grouped by feature, naming the decision. They cost nothing, they run green, and they turn "every
   business rule has a test" from a rule someone has to remember into a list that is visibly
   unfinished. A later phase that implements D16 finds the stub already waiting and fills it in;
   a decision with no stub is a decision someone forgot to carry across.

   **A green suite full of `todo` is exactly the silent success this project distrusts.** So the
   advisory scan reports the count — *"N decisions, M stubbed"* — and `check:rules` prints it every
   run. The number going down is the progress bar; a number that never moves is the finding.

   **The stubs are a template, not a list of strings.** Every one is written in the shape a filled-in
   test will take — build a state, apply an event with a `ctx`, assert the next state and the emitted
   effects — so completing one is filling a body rather than deciding a form. That is what keeps the
   golden traces of §15 consistent across phases written weeks apart, and it is why the template
   lands in Phase 0 rather than being invented, differently, three times.

   **Done when:** the status screen runs on device showing a migrated schema and a working round
   trip, `npm test` **and `npm run check:code`** both pass, **and the purity rule has been proven to
   fire** by adding a deliberately forbidden import to `src/core/types.ts`, watching lint fail, and
   reverting it. A rule believed to be enforced but not actually wired is worse than no rule.

   **The simulator is the gate; the device is the confirmation.** Phase 0 is done when it runs in the
   simulator, because §13's own premise is that steps 1–3 need no Apple account and gating here would
   block the work behind Xcode signing. Run it on the phone as well — it is the thing D36 promises —
   but a provisioning problem is a Phase 1 blocker, not a reason Phase 0 is unfinished.
...
| --- | --- | --- |
| **AlarmKit needs the `com.apple.developer.alarmkit` entitlement to run on a device** | Blocks device builds, not simulator ones | **Confirmed on a simulator:** with only `NSAlarmKitUsageDescription` and no account, entitlement or App Group, iOS presents its real authorisation prompt — so the integration is genuinely developable now. Device builds fail on the missing entitlement, and the capability was reported absent from the Developer Portal. Apply as soon as the account exists and treat the lead time as unknown |
...
| **`expo-alarm-kit` needs an App Group before anything works** — `configure()` returns false without one | Medium — narrower than feared | The module loads and AlarmKit prompts for authorisation without one. What an unconfigured App Group costs is the bridge's own bookkeeping: `getAllAlarms()` reads that storage, so it returns empty and reconciliation must not read that as "nothing is scheduled" |
| **A pod whose deployment target exceeds the app's is skipped silently** | High, and completely silent | Cost an evening to find: the build succeeds, the app runs, and the native module is absent until something calls it. Match the app's target to the strictest pod, and treat a missing native module at runtime as a linking problem before a code one |
...
2. **Every rule in `core/` has a test** (D37), against a synthetic clock — vitest, one tier, no
   ...
   For scale, a mature repo in this stack carries roughly one line of test per three of source.
   ...
   **Tests are co-located** — `src/db/settings.ts` is tested by `src/db/settings.test.ts`. A test
beside its subject is read when the subject changes; one in a parallel tree quietly drifts.
`src/__tests__/` holds only the shared harness and the decision checklist.

**Anything worth testing takes its dependencies as arguments.** `settings.ts` receives a database
handle rather than importing the device one — that is what makes one schema genuinely drive both
drivers instead of merely claiming to.
...
`db.serialize()`, and hand that buffer to `new Database(buffer)` per test. Restoring a migrated database
...
once, beacon lost mid-session, phone restarted, exit-and-return.
...
the mechanism enforcing D36, this is the first thing that would quietly erode them.
...
leaked and the trace says so immediately. Re-recording a trace is how that protection gets lost, so
it needs a stated reason every time.
...
Xcode. The equivalent is EAS Build, or macOS runners at a 10× minute multiplier.
...
- The house-rules scan is **advisory** — always exits 0, never gates. It surfaces conventions that
  ...
  missing co-located `types.ts`, comments interleaved between fields of an object literal,
  `Platform.OS` outside `src/alarm/`, and an exported symbol or a file in `core/` with no doc block.
  Each is text-detectable with acceptable noise; anything needing type information belongs in
  review, not a regex.
...
in CI initially** — macOS runners are expensive and EAS on demand is enough for a solo project.
